...
import { updateClassTemplate } from '../services/scheduling.actions'
import type { ClassTemplate, Discipline } from '../types'
...
export function EditTemplateModal({ template, disciplines, teachers, rooms, students, onClose }: Props) {
  const [pending, startTransition] = useTransition()
  const [error, setError] = useState<string | null>(null)
  const [noRoom, setNoRoom] = useState(!template.room)
  const [studentSearch, setStudentSearch] = useState('')
  const [selectedStudents, setSelectedStudents] = useState<string[]>(template.student_ids ?? [])
  ...
  const [form, setForm] = useState({
    ...
    room: template.room ?? '',
    max_capacity: String(template.max_capacity ?? 20),
  })
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-white/10">
          <h2 className="text-lg font-bold text-white">Editar plantilla</h2>
